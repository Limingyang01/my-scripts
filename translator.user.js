...
(function() {
    ...
    // --- ⚙️ 配置中心 ---
    const DEFAULTS = {
        MODEL: "MiniMax-M3", // M3 支持关闭思考；M2.x 强制思考，只能靠 reasoning_split 分流
        API_URL: "https://api.minimax.cn/v1/chat/completions",
        TARGET_LANG: "简体中文",
        TRANS_STYLE: "daily",
        ICON_OFFSET_X: 10,
        ICON_OFFSET_Y: 10,
    ...
    // 翻译请求体：thinking.disabled 只有 MiniMax-M3 生效；M2.x 强制思考，
    // 靠 reasoning_split 把思考内容分流到 reasoning_content，content 只留译文
    function buildTranslatePayload(text) {
        return JSON.stringify({
            model: config.model,
            messages: [
                { role: "system", content: `You are a translator. Target: ${resolveTargetLang(text)}. Style: ${PROMPT_STYLES[config.transStyle] || PROMPT_STYLES.daily}. Rule: Output ONLY the translated text. No markdown, no explanation, no reasoning.` },
    ...
    // 兜底：模型忽略 reasoning_split 时，思考过程会以 <think> 包裹留在 content 里
    function extractTranslation(data) {
        const raw = data.choices[0].message.content || "";
        const out = raw
            .replace(/<think>[\s\S]*?<\/think>/gi, "")   // 完整思考块
            .replace(/^[\s\S]*?<\/think>/i, "")          // 只有闭合标签（开标签被吞）
            .replace(/<think>[\s\S]*$/i, "")             // 只有开标签（思考被截断）
            .trim();
        if (!out) throw new Error("empty content"); // 思考占满 token，交给调用方报错
        return out;
    }
    ...
    const styles = `
        ...
        .sf-page-trans { display: block; margin-top: 6px; }
        ...
        .sf-toast { border: 1px solid var(--sf-glass-border); background: var(--sf-tooltip-bg-dark); color: var(--sf-tooltip-text); padding: 12px 28px; border-radius: 50px; font-size: 14px; font-weight: 600; display: flex; align-items: center; gap: 10px; opacity: 0; transform: translateY(-30px) scale(0.9); transition: all 0.5s var(--sf-ease-spring); -webkit-font-smoothing: antialiased; z-index: 2147483648; box-sizing: border-box; }
            ...
            <div class="sf-view sf-view-main">
                ...
                <div style="margin-bottom: 24px;">
                    ...
                    <datalist id="sf-model-list">
                        <option value="MiniMax-M3">MiniMax-M3 (推荐：可关思考，最快)</option>
                        <option value="MiniMax-M2.7-highspeed">MiniMax-M2.7 高速 (强制思考)</option>
                        <option value="MiniMax-M2.7">MiniMax-M2.7 (强制思考)</option>
                    </datalist>
                </div>
                <button id="sf-save-btn" class="sf-btn sf-btn-primary">保存更改</button>
                <div style="margin-top:16px; text-align:center;">
                    <a href="https://platform.minimaxi.com/" target="_blank" style="color:var(--sf-primary); font-size:12px; text-decoration:none; opacity:0.8;">获取 API Key</a>
                </div>
                <!-- ℹ️ 底部右下角的信息按钮 -->
    ...
    function showToast(message, type = 'info') {
        ...
        const shader = new LiquidElementShader(toast, {
            ...
            // 跟 tooltip 用同一套深底白字：原来是全透明 + 深色文字，落在深色页面上就糊了
            backgroundColor: 'var(--sf-tooltip-bg-dark)'
        });
    ...
    GM_registerMenuCommand("🌐 翻译整个页面 (Alt+A)", () => translateWholePage());
    GM_registerMenuCommand("↩️ 还原整页原文 (Alt+S)", () => restoreWholePage());
    ...
    function handleManualTranslation(text) {
        ...
        GM_xmlhttpRequest({
            ...
            data: buildTranslatePayload(text),
            onload: function(res) {
                outputArea.style.opacity = "1";
                if (res.status === 200) {
                    try {
                        const data = JSON.parse(res.responseText);
                        outputArea.value = extractTranslation(data);
                    } catch (e) {
    ...
    // --- 🌐 一键整页翻译（双语对照：译文追加在原文下方，不改动原 DOM）---
    const PAGE_BLOCK_SELECTOR = "p, li, h1, h2, h3, h4, h5, h6, blockquote, dd, dt, figcaption, td, th";
    const PAGE_CONFIRM_THRESHOLD = 50; // 超过这么多段先问一句，避免误点烧掉一大把 token
    let pageTranslating = false;

    function collectPageBlocks() {
        return [...document.querySelectorAll(PAGE_BLOCK_SELECTOR)].filter(el => {
            if (el.closest("#sf-settings-modal, #sf-manual-panel, .sf-translated-node")) return false;
            if (el.querySelector(PAGE_BLOCK_SELECTOR)) return false;        // 只取最内层块，避免父子重复翻译
            if (el.querySelector(".sf-page-trans")) return false;           // 已翻译过
            if (!el.offsetParent && el.tagName !== "TD" && el.tagName !== "TH") return false; // 隐藏元素
    ...
    async function runQueue(tasks) {
        let i = 0;
        await Promise.all(Array.from({ length: DEFAULTS.MAX_CONCURRENT }, async () => {
            while (i < tasks.length) {
                await tasks[i++]();
                await new Promise(r => setTimeout(r, DEFAULTS.REQUEST_INTERVAL));
            }
        }));
    }
    ...
    async function translateWholePage() {
        if (!config.apiKey) return toggleSettings(true);
        if (pageTranslating) return showToast("整页翻译进行中…", "info");

        const blocks = collectPageBlocks();
        if (!blocks.length) return showToast("没有找到可翻译的段落", "error");
        if (blocks.length > PAGE_CONFIRM_THRESHOLD &&
            !confirm(`本页共 ${blocks.length} 个段落，将发起 ${blocks.length} 次翻译请求，继续？`)) return;

        pageTranslating = true;
        showToast(`开始翻译 ${blocks.length} 段`, "success");

        const tasks = blocks.map(el => () => new Promise(resolve => {
            const text = el.innerText.trim();
            const span = document.createElement("span");
            span.className = "sf-translated-node sf-page-trans sf-loading";
            span.innerText = text;
            span.setAttribute("data-original", text);
            span.setAttribute("data-state", "loading");
            el.appendChild(span);
            doTranslation(text, span, resolve);
        }));

        await runQueue(tasks);
        pageTranslating = false;
        showToast("整页翻译完成", "success");
    }

    // 还原：译文都是追加的独立节点，直接删掉即可，原文一个字没动过
    function restoreWholePage() {
        const nodes = document.querySelectorAll(".sf-page-trans");
        if (!nodes.length) return showToast("当前没有整页译文", "info");
        nodes.forEach(n => n.remove());
        showToast(`已还原 ${nodes.length} 段`, "success");
    }
    ...
    function doTranslation(text, spanElement, done = () => {}) {
        GM_xmlhttpRequest({
            method: "POST",
            url: DEFAULTS.API_URL,
            timeout: 30000,
            ...
            onload: function(res) {
                if (res.status === 200) {
                    try {
                        const data = JSON.parse(res.responseText);
                        updateUISuccess(spanElement, extractTranslation(data));
                    } catch (e) {
            ...
            onerror: () => { updateUIError(spanElement, "网络错误"); done(); },
            ontimeout: () => { updateUIError(spanElement, "请求超时"); done(); }
        });
    }
    ...
    function updateUISuccess(span, text) {
        ...
        // 整页译文是追加的独立节点，本身就是对照展示，不走"仅悬浮窗"模式
        if (config.onlyTooltip && !span.classList.contains("sf-page-trans")) {
            // --- 仅悬浮窗模式 ---
